    ...
    public class FluenceFlowing {
        ...
        public class Handler : IRequestHandler<Command> {
            ...
            public async Task<Unit> Handle(Command request, CancellationToken cancellationToken) {
                ...
                var etude = user.Etudes.FirstOrDefault(x => x.Id == request.Id && x.AppUserId == user.Id);

                if (etude == null)
                    throw new Errors.RESTException(HttpStatusCode.NotFound, new { etude = "Not Found" });

                if (etude.Fluence == FlowingCode)
                    throw new Errors.RESTException(HttpStatusCode.Forbidden, new { etude = "It´s already this state" });


                if (etude != null && etude.Fluence != FlowingCode) {
                    etude.Fluence = FlowingCode;
                }
